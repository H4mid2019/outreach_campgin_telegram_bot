...
class PresetCampaign(Base):
    ...
    Each campaign has a name (unique slug), description, target text, and JSON email list."""
    ...
    email_list_json: Mapped[str] = mapped_column(Text)  # JSON list of {name,email,info,language}
    created_at: Mapped[datetime] = mapped_column(default=datetime.utcnow)
...
async def get_all_campaigns() -> List[Dict]:
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(PresetCampaign).order_by(PresetCampaign.name))
...
async def upsert_campaign(name: str, description: str, target: str, email_list: List[Dict]) -> bool:
