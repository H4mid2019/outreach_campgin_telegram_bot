...
from keyboards.inline import get_campaign_admin_keyboard, get_campaign_delete_keyboard
...
from utils.campaign_attachments import save_attachment, cleanup_campaign_attachments
...
async def _enter_campaign_key(
    message: Message, state: FSMContext, via: str = "message"
):
...
async def receive_campaign_key(message: Message, state: FSMContext):
    ...
    if entered != Config.CAMPAIGN_ACCESS_KEY:
        await message.answer(
            "❌ کلید نادرست است. دسترسی رد شد.", reply_markup=_back_keyboard()
        )
        await state.clear()
        return
...
@router.callback_query(
    PresetCampaignStates.waiting_action,
    lambda c: c.data and c.data.startswith("camp_admin:"),
)
async def handle_admin_action(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "add":
        ...
        await callback.message.edit_text(
            text, parse_mode="HTML", reply_markup=get_campaign_admin_keyboard()
        )
...
async def confirm_delete_campaign(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(
        text=f"✅ بله، حذف کن «{name}»", callback_data=f"camp_delete_yes:{name}"
    )
...
@router.callback_query(
    PresetCampaignStates.waiting_delete_confirm,
    lambda c: c.data and c.data.startswith("camp_delete_yes:"),
)
async def execute_delete_campaign(callback: CallbackQuery, state: FSMContext):
    name = callback.data.split(":", 1)[1]
    deleted = await delete_campaign(name)
    if deleted:
        cleanup_campaign_attachments(name)
        await callback.message.edit_text(
            f"✅ کمپین <b>{name}</b> (و پیوست‌ها) با موفقیت حذف شد.",
            parse_mode="HTML",
            reply_markup=get_campaign_admin_keyboard(),
        )
    else:
...
async def receive_campaign_email_list_file(message: Message, state: FSMContext):
    ...
    if not file.file_name.endswith(".csv"):
        await message.answer(
            "❌ فقط فایل‌های CSV مجاز است.", reply_markup=_admin_back_keyboard()
        )
        return

    file_info = await message.bot.get_file(file.file_id)
    temp_path = os.path.join(
        tempfile.gettempdir(), f"camp_csv_{message.chat.id}_{file.file_unique_id}.csv"
    )
    ...
    await state.update_data(camp_records=records)
    await _prompt_attachments(message, state)
...
async def receive_campaign_email_list_text(message: Message, state: FSMContext):
    try:
        ...
        if not content:
            await message.answer(
                "❌ داده خالی است.", reply_markup=_admin_back_keyboard()
            )
            return
        ...
        temp_path = os.path.join(
            tempfile.gettempdir(), f"camp_text_{message.chat.id}.csv"
        )
        ...
        if not is_valid:
            await message.answer(
                f"❌ خطای CSV: {msg}", reply_markup=_admin_back_keyboard()
            )
            return
        ...
        await message.answer(
            f"❌ خطا در پردازش: {str(e)}", reply_markup=_admin_back_keyboard()
        )


async def _prompt_attachments(message: Message, state: FSMContext):
    """Prompt for optional attachments after email list."""
    data = await state.get_data()
    name = data["camp_name"]
    existing = await get_campaign_by_name(name)
    att_count = len(existing["attachments"]) if existing else 0

    skip_text = (
        "⏭️ رد کردن (هیچ پیوستی اضافه نشود)"
        if att_count == 0
        else "⏭️ رد کردن (پیوست‌های قبلی حفظ شود)"
    )
    text = (
        f"📎 <b>پیوست‌های اختیاری (حداکثر {Config.MAX_CAMPAIGN_ATTACHMENTS} فایل، {Config.MAX_TOTAL_ATTACHMENTS_MB}MB کل)</b>\n\n"
        f"📌 کمپین: <code>{name}</code>\n"
        f"📎 فعلی: {att_count} فایل\n\n"
        f"✅ آپلود کنید (چندین فایل مجاز) یا:\n"
        f"<code>/skip</code> {skip_text}\n"
        f"<code>/done</code> تمام شد، ذخیره کن"
    )
    await message.answer(text, parse_mode="HTML", reply_markup=_admin_back_keyboard())
    await state.set_state(PresetCampaignStates.waiting_attachments)
    await state.update_data(camp_attachments=[])
...
async def _save_campaign(
    ...
    # Cleanup old attachments if new ones provided (replace)
    existing = await get_campaign_by_name(name)
    if attachments is not None and existing:
        cleanup_campaign_attachments(name)

    was_created = await upsert_campaign(name, description, target, records, attachments)

    action_word = "✅ ایجاد شد" if was_created else "🔄 به‌روزرسانی شد"
    att_note = f"\n📎 پیوست‌ها: <b>{len(attachments or [])}</b>" if attachments else ""

    await message.answer(
        f"{action_word} کمپین <b>{name}</b>{att_note}\n\n"
        f"📝 توضیح: {description}\n"
...
@router.message(PresetCampaignStates.waiting_attachments)
async def receive_campaign_attachments(message: Message, state: FSMContext):
    data = await state.get_data()
    name = data["camp_name"]
    records = data["camp_records"]
    attachments = data.get("camp_attachments", [])

    if message.document:
        # Attachment upload
        file = message.document
        file_size_mb = file.file_size / (1024**2) if file.file_size else 0

        if len(attachments) >= Config.MAX_CAMPAIGN_ATTACHMENTS:
            await message.answer(
                f"❌ حداکثر {Config.MAX_CAMPAIGN_ATTACHMENTS} فایل مجاز است.",
                reply_markup=_admin_back_keyboard(),
            )
            return

        if file_size_mb > Config.MAX_ATTACHMENT_SIZE_MB:
            await message.answer(
                f"❌ فایل بزرگتر از {Config.MAX_ATTACHMENT_SIZE_MB}MB است.",
                reply_markup=_admin_back_keyboard(),
            )
            return

        total_size_mb = sum(att["size_bytes"] for att in attachments) / (1024**2)
        if total_size_mb + file_size_mb > Config.MAX_TOTAL_ATTACHMENTS_MB:
            await message.answer(
                f"❌ کل پیوست‌ها بیش از {Config.MAX_TOTAL_ATTACHMENTS_MB}MB است.",
                reply_markup=_admin_back_keyboard(),
            )
            return
        ...
        file_info = await message.bot.get_file(file.file_id)
        temp_path = os.path.join(
            tempfile.gettempdir(), f"camp_att_{message.chat.id}_{file.file_unique_id}"
        )
        await message.bot.download_file(file_info.file_path, temp_path)

        try:
            att_meta = save_attachment(temp_path, name, file.file_name)
            attachments.append(att_meta)
            await state.update_data(camp_attachments=attachments)
        finally:
            os.unlink(temp_path)
    ...
    if text_lower == "/skip":
        await _save_campaign(message, state, records, None)
    elif text_lower == "/done":
        await _save_campaign(message, state, records, attachments)
    else:
        await message.answer(
            "❌ دستور نامعتبر. /skip یا /done یا آپلود فایل.",
            reply_markup=_admin_back_keyboard(),
        )
